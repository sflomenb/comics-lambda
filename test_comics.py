...
class ComicsTest(unittest.TestCase):
    ...
    def test_website_exists_s3(self):
        ...
        conn.put_object(Bucket='sflomenb-comics', \
                Key='sflomenb-comics', Body='hello')
    ...
    def test_upload_to_s3(self):
        ...
        conn = boto3.client('s3')
        conn.create_bucket(Bucket='sflomenb-comics')
        self.assertFalse(len(conn.list_objects(Bucket='sflomenb-comics', Prefix='sflomenb-comics').get('Contents', [])) > 0)
        upload_to_s3('the content')
        self.assertTrue(len(conn.list_objects(Bucket='sflomenb-comics', Prefix='sflomenb-comics').get('Contents', [])) > 0)
        self.assertEqual(conn.get_object(Bucket='sflomenb-comics', Key='sflomenb-comics').get('Body').read().decode('utf-8'), 'the content')
